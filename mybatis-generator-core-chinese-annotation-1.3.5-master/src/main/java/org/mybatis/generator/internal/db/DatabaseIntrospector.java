...
public class DatabaseIntrospector {
	...
	/**
	 * 连接的数据库名称，MYSQL or ORACLE 用于提取不同数据库的备注信息
	 */
	...
	private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfiguration tc) throws SQLException {
		...
		while (rs.next()) {
			...
			if ("ORACLE".equals(databaseProductName)) {
				...
				ResultSet mrs = stmt
						.executeQuery(new StringBuilder().append("select * from user_col_comments where Table_Name='")
								.append(tc.getTableName() + "' AND COLUMN_NAME='")
								.append(rs.getString("COLUMN_NAME") + "'").toString());
				while (mrs.next())
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			...
			if ("MYSQL".equals(databaseProductName)) {
				...
				ResultSet rs = stmt.executeQuery(
						new StringBuilder().append("select * from user_tab_comments where Table_Name Like '")
								.append(atn.getTableName()).append("'").toString());
				while (rs.next())
	...
	private void calculateUniqueColumns(FullyQualifiedTable table, IntrospectedTable introspectedTable)
			throws SQLException {
		...
		if ("MYSQL".equals(databaseProductName)) {
			...
			// start mysql
			Statement stmt = this.databaseMetaData.getConnection().createStatement();
			ResultSet rs = stmt.executeQuery(new StringBuilder().append("show keys from ")
					.append(table.getIntrospectedTableName()).append("").toString());
			while (rs.next()) {
	...
	/**
	 * This method calls database metadata to retrieve some extra information about
	 * the table such as remarks associated with the table and the type.
	 * <p>
				...
				if (remarks == null || remarks.isEmpty()) {
					if ("ORACLE".equals(databaseProductName)) {
						...
						ResultSet mrs = stmt.executeQuery(
								new StringBuilder().append("select * from user_tab_comments where Table_Name Like '")
										.append(introspectedTable.getFullyQualifiedTable()).append("'").toString());
						while (mrs.next())
						...
						// start mysql
						Statement stmt = this.databaseMetaData.getConnection().createStatement();
						ResultSet nrs = stmt.executeQuery(new StringBuilder().append("SHOW FULL COLUMNS FROM ")
								.append(introspectedTable.getFullyQualifiedTable()).toString());
						while (nrs.next())
